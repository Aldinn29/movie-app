...
export default function Sidebar(props) {
    const { navElements, active, setActive, setModal, setSearchValue, setGenre, setTitle, setCurrentPage } = props
                ...
                {navElements.map((item, index) => (
                    <li key={index} className="nav-item ">
                        <button className={`btn text-white rounded-0 w-100 d-flex flex-column gap-1 py-3 fs-5 ${active === item.title ? 'btn-primary' : ''}`}
                            onClick={() => { setActive(item.title), setModal(null), setSearchValue(''), setGenre(''), setTitle('Genre'), setCurrentPage(1) }}>
